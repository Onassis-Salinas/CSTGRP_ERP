<script lang="ts">
    ...
    import { DotsVerticalOutline, FileExportOutline, PlusCircleOutline, UserAddOutline, UserEditOutline, UserOutline, UserRemoveOutline } from "flowbite-svelte-icons";
...
<div class="flex justify-between mb-4">
    ...
    <div>
        <Button  on:click={exportList}><FileExportOutline class="mr-2 size-4" />Exportar</Button>
        <Button  on:click={createEmployee}><PlusCircleOutline class="mr-2 size-4" />Añadir empleado</Button>
    </div>
</div>
            ...
            <TableBodyRow>
                <TableBodyCell class="px-0 sticky left-0 bg-inherit">
                    ...
                    <Dropdown containerClass="z-[999] flex gap-2" placement="right" class="p-2 flex gap-2">
                        <Button  on:click={() => previewEmployee(i)} color="primary"><UserOutline />Ver</Button>
                        <Button  on:click={() => editEmployee(i)} color="green"><UserEditOutline />Editar</Button>
                        {#if employee.active}
                            <Button  on:click={() => deleteEmployee(i)} color="red"><UserRemoveOutline />Baja</Button>
                        {:else}
                            <Button  on:click={() => reactivateEmployee(i)} color="purple"><UserAddOutline />Recontratar</Button>
                        {/if}
                    </Dropdown>
                </TableBodyCell>
                ...
                <TableBodyCell><Badge color={employee.areaId === "1" ? "dark" : "blue"}>{areas[employee.areaId || ""]}</Badge></TableBodyCell>
