<script lang="ts">
    ...
    async function exportFile() {
        ...
        worksheet.columns = [{ width: 12 }, { width: 14 }, { width: 40 }, { width: 25 }, { width: 12 }, { width: 12 }, { width: 12 }, { width: 12 }, { width: 12 }];

        Object.keys(productivity).forEach((key) => {
            worksheet.addRow([]);
            worksheet.addRow([]);
            worksheet.addRow([areas[key]]);
            worksheet.addRow(["Promedio", "No. Empleado", "Nombre", "Posicion", weekDays[0], weekDays[1], weekDays[2], weekDays[3], weekDays[4]]);

            productivity[key].forEach((row) => {
                const average = (parseInt(row["0average"]) + parseInt(row["1average"]) + parseInt(row["2average"]) + parseInt(row["3average"]) + parseInt(row["4average"])) / 5;
                worksheet.addRow([average, row.noEmpleado, row.name, positions[row.positionId], row["0average"], row["1average"], row["2average"], row["3average"], row["4average"]]);
            });
        });
        ...
        worksheet.eachRow((row) => {
            if (row.cellCount === 1) {
                ...
                worksheet.mergeCells(parseInt(cell.row), parseInt(cell.col), parseInt(cell.row), parseInt(cell.col) + 8);
