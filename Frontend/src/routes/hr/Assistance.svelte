...
<div class="flex justify-between mb-4">
    <div class=" flex gap-2">
        <Select size="sm" class="text-xs" placeholder="Eligir Area" items={areasList} bind:value={areaSelected} />
    ...
    <TableBody tableBodyClass="divide-y">
        {#each filteredAssistances as assistance, i}
            <TableBodyRow>
                ...
                <TableBodyCell><Badge color={assistance.areaId === "1" ? "dark" : "indigo"}>{areas[assistance.areaId || ""]}</Badge></TableBodyCell>
                <TableBodyCell><Badge color="purple">{positions[assistance.positionId || ""] || ""}</Badge></TableBodyCell>
                <TableBodyCell tdClass="px-1">
                    <Select size="sm" items={incidencesList} bind:value={filteredAssistances[i].incidenceId0} on:change={() => editAssistance(i)} />
                </TableBodyCell>
                <TableBodyCell tdClass="px-1">
                    <Select size="sm" items={incidencesList} bind:value={filteredAssistances[i].incidenceId1} on:change={() => editAssistance(i)} />
                </TableBodyCell>
                <TableBodyCell tdClass="px-1">
                    <Select size="sm" items={incidencesList} bind:value={filteredAssistances[i].incidenceId2} on:change={() => editAssistance(i)} />
                </TableBodyCell>
                <TableBodyCell tdClass="px-1">
                    <Select size="sm" items={incidencesList} bind:value={filteredAssistances[i].incidenceId3} on:change={() => editAssistance(i)} />
                </TableBodyCell>
                <TableBodyCell tdClass="px-1">
                    <Select size="sm" items={incidencesList} bind:value={filteredAssistances[i].incidenceId4} on:change={() => editAssistance(i)} />
                </TableBodyCell>
            </TableBodyRow>
        {/each}
        {#if !filteredAssistances[0]}
            <TableBodyRow>
                <TableBodyCell tdClass="col-span-9 h-10" class="col-span-9 h-10"></TableBodyCell>
                <TableBodyCell tdClass="col-span-9 h-10" class="col-span-9 h-10"></TableBodyCell>
                <TableBodyCell tdClass="col-span-9 h-10" class="col-span-9 h-10"></TableBodyCell>
                <TableBodyCell tdClass="col-span-9 h-10" class="col-span-9 h-10"></TableBodyCell>
                <TableBodyCell tdClass="col-span-9 h-10" class="col-span-9 h-10"></TableBodyCell>
